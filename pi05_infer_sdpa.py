from pi05_attention_sdpa import ensure_sdpa_buffers, sdpa_mqa_decoder, sdpa_mqa_encoder
...
def transformer_encoder_sdpa(weights, buffers, encoder_seq_len):
    layer_norm_matmul_n256_1152_2048_bias(
        buffers["vision_x"],
    ...
    for i in range(18):
        ...
        if i != 17:
            sdpa_mqa_encoder(buffers, i, encoder_seq_len)
            matmul_n_2048_2048_res(
...
def transformer_decoder_sdpa(weights, buffers, encoder_seq_len, num_steps=10):
    for step in range(num_steps):
        ...
        for i in range(18):
            ...
            sdpa_mqa_decoder(buffers, i, encoder_seq_len, seq_len)
            matmul_k_2048_1024_gate(
...
def pi05_model_sdpa(weights, buffers, num_views, encoder_seq_len, num_steps=10):
    vision_encoder(weights, buffers, num_views)
    transformer_encoder_sdpa(weights, buffers, encoder_seq_len)
    transformer_decoder_sdpa(weights, buffers, encoder_seq_len, num_steps)


class Pi05SdpaInference(Pi05Inference):
    def record_infer_graph(self):
        ensure_sdpa_buffers(self.buffers, self.encoder_seq_len, self.chunk_size)
        super().record_infer_graph()

    def record_run(self):
        pi05_model_sdpa(self.weights, self.buffers, self.num_views, self.encoder_seq_len)
